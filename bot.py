...
def analyze_coin(symbol):
    ...
    if len(closes_5m) < 100:
        return None
    closes_1h, _, _, _ = get_klines(symbol, '1h', 100)
    rsi_5m = calculate_rsi(closes_5m, RSI_PERIOD)
    rsi_1h = calculate_rsi(closes_1h, RSI_PERIOD) if len(closes_1h) >= RSI_PERIOD else None
...
def start(message):
    ...
    bot.send_message(message.chat.id, "Управление ботом. Нажмите кнопку:", reply_markup=markup)
